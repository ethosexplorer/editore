...
import { Globe, ArrowRightLeft, Copy, Volume2, Zap, Upload, FileText, Trash2, Settings, Star, Lock, Download, Eye, RotateCcw } from 'lucide-react';
...
const TranslatorPage: React.FC = () => {
  ...
  const handleTranslate = async () => {
    ...
    setTimeout(() => {
      ...
      ].filter(alt => alt !== translatedText).slice(0, 3);
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm text-gray-600">{charactersUsed.toLocaleString()} / {characterLimit.toLocaleString()}</span>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">Translation Mode</h3>
            <span className="text-sm text-gray-500">Choose your preferred style</span>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {translationModes.map((mode) => {
              ...
              return (
                <button
                  key={mode.id}
                  onClick={() => mode.premium && !showPremiumModal ? setShowPremiumModal(true) : setTranslationMode(mode.id)}
                ...
                >
                  <div className="flex items-center justify-center mb-2">
                    <IconComponent className={`w-5 h-5 ${translationMode === mode.id ? 'text-indigo-600' : 'text-gray-600'}`} />
                    {mode.premium && (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <div className="flex items-center justify-center space-x-4">
            ...
            <button
              ...
              className="mt-6 p-3 bg-gray-100 hover:bg-gray-200 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  ...
                  <h2 className="text-lg font-semibold text-gray-900">{getLanguageName(sourceLang)}</h2>
                </div>
              ...
              <textarea
                ...
                className="w-full h-80 p-4 border border-gray-300 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              />
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                <button
                  ...
                  className="px-6 py-3 bg-indigo-500 text-white font-medium rounded-lg hover:bg-indigo-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                >
                  {isTranslating ? (
                    <>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                  ...
                  <h2 className="text-lg font-semibold text-gray-900">{getLanguageName(targetLang)}</h2>
                </div>
                <div className="flex space-x-2">
                  {result && (
                    <>
              ...
              {result && (
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <div className="text-center p-3 bg-green-50 rounded-lg border border-green-200">
                    <div className="text-lg font-bold text-green-600">{result.accuracy.toFixed(0)}%</div>
                    <div className="text-xs text-green-600">Accuracy</div>
                  </div>
                  <div className="text-center p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <div className="text-lg font-bold text-blue-600">{result.fluency.toFixed(0)}%</div>
              ...
              {result && result.romanization && (
                <div className="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-yellow-800">Romanization</span>
                    <button
                  ...
                  <p className="text-sm text-yellow-700">{result.romanization}</p>
                </div>
              )}
              ...
              {result && showAlternatives && result.alternatives.length > 0 && (
                <div className="mt-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
                  <h4 className="font-medium text-purple-900 mb-3">Alternative Translations</h4>
                  <div className="space-y-2">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-3">
            {quickPhrases.map((phrase, index) => (
              <button
                ...
                className="p-3 text-left border border-gray-200 rounded-lg hover:border-indigo-300 hover:bg-indigo-50 transition-all text-sm"
              >
                <span className="text-gray-900">{phrase.en}</span>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-md w-full p-6">
              <div className="text-center mb-6">
                ...
                <p className="text-gray-600">Unlock unlimited translations and advanced features</p>
              </div>
              
              <div className="space-y-4 mb-6">
                <div className="flex items-center space-x-3">
                  <Globe className="w-5 h-5 text-green-500" />
                  <span className="text-sm">Unlimited character translations</span>
                </div>
                <div className="flex items-center space-x-3">
                  <Star className="w-5 h-5 text-green-500" />
                  <span className="text-sm">9 premium translation modes</span>
                </div>
                <div className="flex items-center space-x-3">
                  <Settings className="w-5 h-5 text-green-500" />
                  <span className="text-sm">Custom tone and style options</span>
                </div>
                <div className="flex items-center space-x-3">
                  <FileText className="w-5 h-5 text-green-500" />
                  <span className="text-sm">Bulk document translation</span>
                </div>
              </div>
              ...
              <div className="flex space-x-3">
                ...
                <button className="flex-1 px-4 py-3 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-lg hover:opacity-90 transition-opacity">
                  Upgrade Now
                </button>
              </div>
            </div>
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-6">
          <div className="text-center p-6 bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            <h3 className="font-semibold text-gray-900 mb-2">52+ Languages</h3>
            <p className="text-sm text-gray-600">Comprehensive language support with cultural context awareness</p>
          </div>
          <div className="text-center p-6 bg-white rounded-xl shadow-sm border border-gray-200">
            <div className="w-12 h-12 bg-purple-100 rounded-lg flex items-center justify-center mx-auto mb-4">
              <Zap className="w-6 h-6 text-purple-600" />
            </div>
            <h3 className="font-semibold text-gray-900 mb-2">Context-Aware AI</h3>
            <p className="text-sm text-gray-600">Preserves tone, style, and cultural nuances in translations</p>
          </div>
          <div className="text-center p-6 bg-white rounded-xl shadow-sm border border-gray-200">
            <div className="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center mx-auto mb-4">
              <Volume2 className="w-6 h-6 text-green-600" />
            </div>
            <h3 className="font-semibold text-gray-900 mb-2">Audio & Romanization</h3>
            <p className="text-sm text-gray-600">Text-to-speech and romanization for Chinese and Hindi</p>
          </div>
        </div>
      </div>
    </div>
