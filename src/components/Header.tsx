      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center space-x-2 z-50">
            <motion.div 
              className="w-7 h-7 lg:w-8 lg:h-8 bg-gradient-to-r from-blue-500 to-teal-400 rounded-lg flex items-center justify-center"
              whileHover={{ scale: 1.1, rotate: 5 }}
              whileTap={{ scale: 0.95 }}
            >
              <Edit3 className="w-4 h-4 lg:w-5 lg:h-5 text-white" />
            </motion.div>
            <span className="text-lg lg:text-xl font-bold text-gray-900">Editore</span>
          </Link>
          ...
          <nav className="hidden xl:flex items-center space-x-6 2xl:space-x-8">
            {menuItems.map((item) => (
              ...
              >
                {item.name}
                <div className={`absolute -bottom-1 left-3 right-3 h-0.5 bg-gradient-to-r from-teal-400 to-blue-500 transition-all duration-300 ${
                  activeItem === item.name || location.pathname === item.path ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0'
                }`} />
          ...
          >
            ...
            >
              ...
              <nav className="px-4 sm:px-6 py-6">
                <div className="grid gap-1 max-h-[calc(100vh-200px)] overflow-y-auto">
